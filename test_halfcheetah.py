...
def get_args():
    ...
    parser.add_argument('--training-num', type=int, default=60)
...
def test_ppo(args=get_args()):
    ...
    env = RandomWrapper(MetaWrapper(gym.make(args.task)))
    train_envs = env
    test_envs = env
    args.training_num = 1
    args.test_num = 5
    ...
    if args.recurrent:
        ...
        if args.training_num > 1:
            ...
            buffer = ReplayBuffer(args.buffer_size, stack_num=args.history_len)
    dynamic = MetaDynamic(args.oarc_shape, args.action_shape,
                          context_hidden_size=args.context_hidden_size,
                          context_size=args.context_size,
                          hidden_sizes=args.dyn_hidden_sizes)
    ...
    if args.resume:
        ...
        dynamic_optim.load_state_dict(checkpoint['dynamic_optim'])
