...
def get_args():
    ...
    parser.add_argument('--training-num', type=int, default=25)
...
def test_ppo(args=get_args()):
    ...
    if args.task == 'Exhx5WalkMod-v0':
        ...
        if args.watch:
            ...
            train_envs, test_envs = make_random_env(task=Exhx5WalkEnv, wrapper=RandomNoContext,
                                                    train_task_num=args.training_num,
                                                    test_task_num=args.test_num, seed=args.seed)
    elif args.task == 'Exhx5Gazebo-v0':
        env = RandomNoContext(MetaWrapper(Exhx5GazeboEnv()))
        train_envs = env
        test_envs = env
        args.training_num = 1
        args.test_num = 5
    ...
    log_path = os.path.join(args.logdir, args.task, 'ppo_pre_train_without_context_world_model', log_file)
    # if args.resume:
