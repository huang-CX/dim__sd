...
def test_ppo(args=get_args()):
    ...
    if args.task == 'Exhx5WalkMod-v0':
        env = RandomWrapper(MetaWrapper(Exhx5WalkEnv(render=args.watch)))
        if args.watch:
            train_envs = env
            test_envs = env
        else:
            train_envs, test_envs = make_random_env(task=Exhx5WalkEnv, wrapper=RandomWrapper,
    ...
    log_path = os.path.join(args.logdir, args.task, 'ppo_pre_train_with_context_world_model_epoch_500', log_file)
    # if args.resume:
