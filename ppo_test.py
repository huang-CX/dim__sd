...
from env.env_wrapper import RandomWrapper, MetaWrapper, RandomDynamicWrapper, RandomFrictionWrapper
from env.make_random_env import make_random_env, make_random_dynamic_adapt_env, make_random_friction_adapt_env
...
def get_args():
    ...
    parser.add_argument('--task', type=str, default='Exhx5Gazebo-v0')
...
def test_ppo(args=get_args()):
    ...
    if args.task == 'Exhx5WalkMod-v0':
        env = RandomFrictionWrapper(MetaWrapper(Exhx5WalkEnv(render=args.watch)))
        if args.watch:
            train_envs = env
            test_envs = env
        else:
            train_envs, test_envs = make_random_friction_adapt_env(task=Exhx5WalkEnv, wrapper=RandomFrictionWrapper,
