...
from env.env_wrapper import MetaWrapper
from env.make_random_env import make_random_env
from model.net import MQLRecurrentCritic, MQLRecurrentActorProb
...
def test_sac(args=get_args()):
    if args.watch:
        env = MetaWrapper(Exhx5WalkEnv(render=True))
        train_envs = env
        test_envs = env
        args.training_num = 1
        args.test_num = 1
    else:
        env = MetaWrapper(Exhx5WalkEnv())
        train_envs, test_envs = make_random_env(task=Exhx5WalkEnv, wrapper=MetaWrapper,
    ...
    critic1 = MQLRecurrentCritic(layer_num=args.layer_num, state_shape=args.state_shape, action_shape=args.action_shape,
    ...
    critic2 = MQLRecurrentCritic(layer_num=args.layer_num, state_shape=args.state_shape, action_shape=args.action_shape,
                                 device=args.device, context_hidden_size=args.context_hidden_size,
                                 context_size=args.context_size,
                                 hidden_sizes=args.hidden_sizes).to(args.device)
    ...
    if not args.resume_path:
        ...
        log_file = f'seed_{args.seed}_{t0}-{args.task.replace("-", "_")}_mqlsac'
        log_path = os.path.join(args.logdir, args.task, 'mql_sac', log_file)
