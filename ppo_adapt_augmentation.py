...
from env.env_wrapper import MetaWrapper, RandomNoContextFrictionWrapper, RandomNoContextVelocityWrapper, \
    RandomNoContextCoMWrapper, RandomNoContextDynamicWrapper, RandomNoContextIntervalWrapper, \
    RandomNoContextStiffnessWrapper, RandomNoContextDampingWrapper
...
from model.baseline_policy import BaselinePolicy
...
def get_args():
    ...
    parser.add_argument('--step-per-epoch', type=int, default=1024)
    ...
    parser.add_argument('--resume-path', type=str,
                        default='log/Exhx5WalkMod-v0/ppo_pre_train_without_context_world_model/ppo_pre_train_without_context_world_model')
    parser.add_argument('--adapt', type=int, default=True)
    parser.add_argument('--augmentation-ratio', type=int, default=4)
    parser.add_argument(
        '--watch',
        default=False,
        action='store_true',
...
def test_ppo(args=get_args()):
    ...
    if args.subtask == 'friction':
        env_wrapper = RandomNoContextFrictionWrapper
    elif args.subtask == 'velocity':
        env_wrapper = RandomNoContextVelocityWrapper
    elif args.subtask == 'CoM':
        env_wrapper = RandomNoContextCoMWrapper
    elif args.subtask == 'interval':
        env_wrapper = RandomNoContextIntervalWrapper
    elif args.subtask == 'damping':
        env_wrapper = RandomNoContextDampingWrapper
    elif args.subtask == 'stiffness':
        env_wrapper = RandomNoContextStiffnessWrapper
    else:
        env_wrapper = None
    ...
    log_path = os.path.join(args.logdir, args.task, 'ICM_reweight_' + str(args.subtask) + '_baseline', log_file)
    # if args.resume:
